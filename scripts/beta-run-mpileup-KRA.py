...
ind = filter(lambda x: x.endswith(".sorted.bam"), li)
ind.sort()

# KRA commented out 17 August 2018; this line below results in uninformative column names in the mpileup header
#a = map(lambda x: ["Cov-"+'-'.join(x.split('-')[:-1]).replace('lib','').replace('Lib',''),'Call-'+'-'.join(x.split('-')[:-1]).replace('lib','').replace('Lib',''),'Qual-'+'-'.join(x.split('-')[:-1]).replace('lib','').replace('Lib','')], ind)

# KRA added the line below on 17 August 2018; this line catches library identifiers from the file names
a = map(lambda x: ["Cov-"+'-'.join(x.split('.sorted.bam')[:-1]).replace('lib','').replace('Lib',''),'Call-'+'-'.join(x.split('.sorted.bam')[:-1]).replace('lib','').replace('Lib',''),'Qual-'+'-'.join(x.split('.sorted.bam')[:-1]).replace('lib','').replace('Lib','')], ind)
...
for x in zip(regions, outputs):
   ...
   #line = opt.pathSAM + " mpileup -l "+x[0]+" -d "+ maxdepth +" -Q "+basequal+" -f "+file+" "+runlinebams+" > "+x[1]
