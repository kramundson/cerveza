...
parser.add_option("--minCov", "-v", dest="mincov", type = "int", default=10, help="Minimum position coverage to be considered as a vaild position. (10)")
...
class MyThread (multiprocessing.Process):
   ...
   def __init__ (self, filen, res, startpos, endpos, args, vlist):
      self.q = Queue()
      self.filen = filen
      self.res = res
...
for x in cutset:
   countert+=1
   print countert
   threads[countert] = MyThread(filename, countert, x[0], x[1], args, validlist)
...
for set in allcounters:
   for label in sumcounters.keys():
      sumcounters[label] += set[label]
